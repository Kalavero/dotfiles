#!/usr/bin/env python3
"""Structural verifier for /app/PLAN.md (planning-parallel-breakdown).

Same tolerant-parser/strict-elements approach as the feature-breakdown
verifier, plus two checks for the concurrency dimension of the spec.
Checks and weights (each documented at its site):

  a. task_count      0.05  >= 4 distinct tasks (scaled below that)
  ...
  e. files_budget    0.15  every task lists the files it touches and no
                           task lists more than 5 (XL-task rejection)
  f. checkpoints     0.05  >= 2 explicit checkpoint blocks (scaled below)
  g. dag             0.10  dependency sanity: referenced tasks exist,
                           acyclic, at least one real edge
  h. verticality     0.05  at least one task spans 2+ non-test layers
  i. parallelization 0.10  the plan identifies parallelization: 0.5 for a
                           parallelization/sequencing section, 0.5 for
                           >= 2 tasks carrying parallel annotations
  j. shared_blocking 0.10  the migration/shared-schema task is marked as
                           blocking or sequential AND/OR is a dependency
                           of >= 2 parallel slices (full credit needs both
                           the explicit call-out and the edges)
...
WEIGHTS = {
    ...
    "dag": 0.10,
    "verticality": 0.05,
    "parallelization": 0.10,
    "shared_blocking": 0.10,
}
...
FIELD_LABEL_RE = re.compile(
    r"(?i)^\s*(?:[-*]\s*)?(?:\*\*)?\s*"
    r"(dependencies|verification|acceptance|estimated|estimate|scope|description|notes?|risks?|parallelizable)\b"
)
BACKTICK_RE = re.compile(r"`([^`\n]+)`")
BARE_PATH_RE = re.compile(
    r"[\w.\-]+(?:/[\w.\-]+)*\.(?:py|sql|md|txt|toml|cfg|ini|ya?ml|json|sh|csv|html|css|js|ts|rb)\b"
)
# A section heading whose title mentions parallel work.
PARALLEL_SECTION_RE = re.compile(
    r"(?im)^\s*(?:#{1,6}\s+|(?:[-*]\s*)?\*\*)[^\n]*\b(parallel|concurren)"
)
# Markers that a task is explicitly NOT parallelizable / must go first.
BLOCKING_RE = re.compile(
    r"(?i)not paralleli[sz]able|blocking|sequential|must (be )?"
    r"(complete[ds]?|done|finish(ed)?|run|land(ed)?|merge[ds]?)( first| before)\b"
    r"|before (any|all|the) (other|parallel)|prerequisite|foundational"
)
MIGRATION_PATH_RE = re.compile(r"(?i)migration|\.sql\b")
...
def main() -> None:
    ...
    for task_id, block in tasks:
        ...
        parallel_note = bool(re.search(r"(?i)parallel", block))
        if parallel_note:
            annotated_parallel += 1
        ...
        per_task.append(
            {
                ...
                "deps": sorted(refs),
                "parallel_annotation": parallel_note,
                "files": files,
    ...
    # (i) parallelization identified: a sequencing/parallel section plus
    #     per-task annotations, half credit each.
    has_parallel_section = bool(PARALLEL_SECTION_RE.search(text))
    parallel_score = 0.5 * has_parallel_section + 0.5 * min(1.0, annotated_parallel / 2)

    # (j) the shared migration task must be called out as blocking and/or
    #     be a dependency of >= 2 other (parallel) tasks.
    migration_tasks = [
        t for t in per_task if any(MIGRATION_PATH_RE.search(f) for f in t["files"])
    ]
    blocking_detail = {"migration_tasks": [t["id"] for t in migration_tasks]}
    if migration_tasks:
        mt = migration_tasks[0]
        block_text = dict(tasks)[mt["id"]]
        marked = bool(BLOCKING_RE.search(block_text))
        dependents = sum(
            1 for t in per_task if t["id"] != mt["id"] and mt["id"] in t["deps"]
        )
        blocking_detail.update({"marked_blocking": marked, "dependents": dependents})
        if marked and dependents >= 2:
            blocking_score = 1.0
        elif dependents >= 2:
            blocking_score = 0.7
        elif marked:
        ...
        blocking_detail["error"] = "no task touches a migration/.sql file"
        blocking_score = 0.0
    ...
    checks = {
        ...
        "parallelization": round(parallel_score, 3),
        "shared_blocking": blocking_score,
    }
        ...
        json.dumps(
            {
                ...
                "shared_blocking": blocking_detail,
                "tasks": per_task,
            },
            indent=2,
        )
