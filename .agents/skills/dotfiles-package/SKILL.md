...
- Put new tool configuration in the existing package for that tool's domain. Create a package only for a genuinely new tool with its own config tree. Add a new package to `stow-packages.txt` and the package tables in `README.md` and `CLAUDE.md`.
