...
2. **Verify in parallel**: once DONE, launch `kalavero:test-engineer` (coverage gaps on the diff) and `kalavero:code-reviewer` (verdict on the diff) in a single message so they run concurrently. Give both the task's acceptance criteria and the list of changed files.
3. **Gate**:
...
4. Non-critical test-engineer recommendations and reviewer Suggestions: collect them in a running list instead of blocking the loop.
