...
3. **Diagnose each distinct failure** following the kalavero:debugging-and-error-recovery skill — reproduce, localize, root-cause. Fix the cause.
...
- A test that fails intermittently rather than deterministically is a flake, not a fix target — suggest `/kalavero:flaky-spec` rather than looping on it.
