...
4. **Verify**: run the task's verification commands, including affected tests at minimum plus build/lint if the project has them. If the plan names a full-suite check, run it. When the task affects Ruby, Rails, RSpec, or Packwerk files, run `ruby_qa` before reporting; any failed applicable check must be fixed or reported as BLOCKED.
...
- **Ruby quality gate**: `ruby_qa` is required only for tasks affecting Ruby, Rails, RSpec, or Packwerk files. It supplements the task's verification commands; it never replaces them or a required full-suite check.
